...
El runner de `evaluador-web` es complementario y no reemplaza al agente de IA.
...
| Modo dual: runner + LLM en la web | Juan | **PENDIENTE DE PRODUCTIZACIÓN, NO BLOQUEANTE PARA EL REPO** | Sería la vía para convertir la web en evaluador oficial con paridad conceptual al agente. La web actual se mantiene gratuita/offline y ahora declara claramente su alcance. |
...
Sus aportes válidos quedaron absorbidos en la V5 o en este hardening post-merge. Una vez mergeado el PR final de hardening, el PR #14 puede cerrarse como **superseded**, conservando su historial y autoría.

## Estado de la candidata

- Agente/rúbrica V5: **sin cambios normativos**.
- Fixtures: **82 / 9 / 31**.
- Repo externo del contrato: **98 / 98 A/B**.
- Runner: **complementario y explícitamente identificado**.
- CI: incluye build, fixtures, integridad, dos smoke tests reales y comparación runner/contrato.
- Calibración humana: limitación original declarada + validación independiente de Guillermo **85/100**.
- PR #14: listo para quedar superseded después del merge de este hardening.
